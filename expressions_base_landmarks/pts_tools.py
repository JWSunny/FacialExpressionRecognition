...
DATA_DIR = "../data/original/ibug"
TARGET_DIR = "../data/target/ibug"
...
def get_valid_box(image, points):
    ...
    ######  图片处理存储后；观察发现 method 1 针对 ibug 数据集的处理更为有效；(这表调试时，进行了注释操作)
...
def preview(point_file):
    ...
    for i in range(len(raw_points)):
        point = [int(raw_points[i][0] - facebox[0]), int(raw_points[i][1] - facebox[1])]
        new_points.append(point)
    # # #####  裁剪的输入人脸 ；人脸中对应的关键点；  点 <---> 人脸  标注问题；
    draw_landmark_point(face_area, new_points)
    ...
    ### save the result of facial picture   将图片保存到对应的文件夹
    if os.path.exists(img_jpg):
        head, tail = os.path.split(img_jpg)
        image_url = os.path.join(TARGET_DIR, '-' + tail)
    else:
        head, tail = os.path.split(img_png)
        image_url = os.path.join(TARGET_DIR, '-' + tail)

    # cv2.imwrite(image_url, face_area)
